...
public class DungeonGenerator : MonoBehaviour {
        ...
        for (int i = 0; i < rooms.Count; i++) {
            for (int j = 0; j < rooms[i].doors.Count; j++) {
                if (rooms[i].doors[j].door == null) {
                    ...
                    //
                    d.gameObject.transform.position = rooms[i].doors[j].transform.position + this.gameObject.transform.position;
    ...
    private byte timerA = 100;
    private float prev_roomsCount;
    public bool doneGenerating;

    public void Update() {

        if (rooms.Count == prev_roomsCount) timerA = 100;
        else timerA--;

        if(timerA == 0 ) doneGenerating = true;

        if (Input.GetKeyDown(KeyCode.F3)) {
            SceneManager.LoadScene(0,LoadSceneMode.Single);
        }

        prev_roomsCount = rooms.Count;
    }
}
